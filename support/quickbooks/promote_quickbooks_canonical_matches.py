"""Conservative QuickBooks canonical promotion: safe batch aliases and direct contexts.

Reads quickbooks_tables_fields_classification.csv, promotes high-confidence direct_alias_match
and description_alias_match rows into the canonical layer, and produces a detailed report.

Safe batch rules:
- Direct field → canonical concept alias: promote only direct_alias_match rows.
- Description → canonical concept: promote description_alias_match rows where concept is unambiguous.
...
PROMOTED_REPORT_PATH = PROJECT_ROOT / "knowledge_sources" / "generated" / "runtime" / "quickbooks" / "quickbooks_promoted_canonical_aliases.csv"
REVIEW_QUEUE_PATH = PROJECT_ROOT / "knowledge_sources" / "generated" / "runtime" / "quickbooks" / "quickbooks_promotion_review_queue.csv"
SUMMARY_PATH = PROJECT_ROOT / "knowledge_sources" / "generated" / "runtime" / "quickbooks" / "quickbooks_promotion_summary.csv"
...
def main(dry_run: bool = False) -> None:
    ...
    for row in rows:
        ...
        elif bucket == "description_alias_match" and concept_id:
            promoted.append(row)
            promotion_counts["description_alias_match"] += 1
        else:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Promote high-confidence QuickBooks canonical matches into the canonical layer.")

"""Conservative Workday webservice canonical promotion (wave-1).

Reads workday_webservice_classification.csv and promotes direct + description
alias matches, then writes review queue and summary reports.

Usage:
    python support/workday/promote_workday_webservice_canonical_matches.py [--dry-run]
"""
...
PROMOTED_REPORT_PATH = PROJECT_ROOT / "knowledge_sources" / "generated" / "runtime" / "workday" / "workday_webservice_promoted_canonical_aliases.csv"
REVIEW_QUEUE_PATH = PROJECT_ROOT / "knowledge_sources" / "generated" / "runtime" / "workday" / "workday_webservice_promotion_review_queue.csv"
SUMMARY_PATH = PROJECT_ROOT / "knowledge_sources" / "generated" / "runtime" / "workday" / "workday_webservice_promotion_summary.csv"
...
def main(dry_run: bool = False) -> None:
    ...
    for row in rows:
        ...
        direct_id = row.get("direct_field_canonical_concept_id", "").strip()
        description_id = row.get("description_canonical_concept_id", "").strip()

        if bucket == "direct_alias_match" and direct_id:
            promoted.append(row)
            promotion_counts["direct_alias_match"] += 1
        elif bucket == "description_alias_match" and description_id:
            promoted.append(row)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Promote high-confidence Workday webservice canonical matches.")

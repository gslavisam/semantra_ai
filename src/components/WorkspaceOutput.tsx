...
export const WorkspaceOutput: React.FC<WorkspaceOutputProps> = ({ 
  ...
  const [activeCodeTab, setActiveCodeTab] = useState<'pandas' | 'pyspark' | 'dbt' | 'dbt_test' | 'great_expectations' | 'sql_audit' | 'talend' | 'azure_sql' | 'openlineage' | 'datacontract'>('pandas');
  ...
  // Data Contract & SLA Policy State
  const [contractDomain, setContractDomain] = useState('Finance & Revenue Mesh');
  const [contractOwner, setContractOwner] = useState('data-governance-lead@enterprise.corp');
  const [contractFreshness, setContractFreshness] = useState('15m max latency (Near Real-Time)');
  const [contractAvailability, setContractAvailability] = useState('99.95% uptime');
  const [contractSchemaEvolution, setContractSchemaEvolution] = useState<'strict' | 'additive_only' | 'compatible'>('strict');
  const [isContractValidating, setIsContractValidating] = useState(false);
  const [contractValidationSuccess, setContractValidationSuccess] = useState(true);

  // GitOps & CI/CD Export State
  const [isGitOpsModalOpen, setIsGitOpsModalOpen] = useState(false);
  const [gitProvider, setGitProvider] = useState<'github' | 'gitlab' | 'azure_devops'>('github');
  const [gitRepo, setGitRepo] = useState('enterprise-data-mesh/core-data-pipelines');
  const [gitBranch, setGitBranch] = useState(`feature/semantra-contract-${selectedPreset ? selectedPreset.toLowerCase().replace(/[^a-z0-9_]/g, '_') : 'canonical'}`);
  const [gitCommitMsg, setGitCommitMsg] = useState(`feat: publish Semantra Data Contract & dbt models for ${selectedPreset || 'Canonical'}`);
  const [gitSelectedTab, setGitSelectedTab] = useState<'workflow' | 'pr_preview' | 'artifacts'>('workflow');
  const [gitPrStatus, setGitPrStatus] = useState<'idle' | 'submitting' | 'created'>('idle');
...
${mappings.map(m => `                {"source": "${m.sourceField}", "target": "${m.targetField}", "rule": "${m.transformation || 'DIRECT_COPY'}"}`).join(',\n')}
  ...
  const getDataContractYaml = () => {
    const datasetSlug = selectedPreset ? selectedPreset.toLowerCase().replace(/[^a-z0-9_]/g, '_') : 'canonical_data_mesh';
    const tableName = `${datasetSlug}_golden`;

    return `# ==============================================================================
# OpenDataContract Specification v2.2.0
# Generated by: Semantra Semantic Control Plane & Governance Compiler
# Dataset: ${selectedPreset || 'Canonical Golden Record'}
# Standard: https://datacontract.com
# ==============================================================================
dataContractSpecification: 2.2.0
id: urn:datacontract:enterprise:${datasetSlug}
info:
  title: ${selectedPreset || 'Enterprise'} Master Data Contract
  version: 1.0.0
  status: ACTIVE
  description: Official semantic data contract defining schema invariants, SLA guarantees, and stewardship ownership.
  owner: ${contractOwner}
  contact:
    name: ${contractDomain}
    email: ${contractOwner}
...
# ------------------------------------------------------------------------------
# Service Level Agreement (SLA) & Quality Guarantees
# ------------------------------------------------------------------------------
serviceLevelAgreement:
  freshness:
    maxLatency: "${contractFreshness}"
    cronSchedule: "0 */4 * * *"
  availability:
    percentage: "${contractAvailability}"
    maintenanceWindow: "Sundays 02:00-04:00 UTC"
  retention:
    period: "7 years"
    compliance: "GDPR Art 17 & Financial Regulatory Audit Archive"
  support:
    tier: "Tier-1 Enterprise Mission Critical"
    responseTime: "15 minutes on breaking schema breach"

# ------------------------------------------------------------------------------
# Schema Evolution & Governance Policies (Zero Breaking Changes)
# ------------------------------------------------------------------------------
schemaEvolution:
  mode: ${contractSchemaEvolution}
  breakingChangesAllowed: false
  quarantinePolicy:
    routeViolationsToDlq: true
    dlqTopic: "enterprise.dlq.${datasetSlug}.schema_drift"

# ------------------------------------------------------------------------------
# Models & Semantic Column Definitions
# ------------------------------------------------------------------------------
models:
  ${tableName}:
    type: table
    description: Golden canonical entity reconciled across source systems.
    fields:
${mappings.map((m, idx) => {
  const isPk = idx === 0 || m.targetField.toLowerCase().includes('id');
  const cleanType = (m.targetType || 'VARCHAR').toLowerCase().includes('int') ? 'integer'
    : (m.targetType || 'VARCHAR').toLowerCase().includes('date') ? 'date'
    : (m.targetType || 'VARCHAR').toLowerCase().includes('dec') || (m.targetType || 'VARCHAR').toLowerCase().includes('num') ? 'number'
    : (m.targetType || 'VARCHAR').toLowerCase().includes('bool') ? 'boolean'
    : 'string';
  const isPii = m.targetField.toLowerCase().includes('email') || m.targetField.toLowerCase().includes('tax') || m.targetField.toLowerCase().includes('phone') || m.targetField.toLowerCase().includes('ssn');

  return `      - name: ${m.targetField}
        type: ${cleanType}
        primaryKey: ${isPk}
        required: ${isPk || m.score > 0.85}
        unique: ${isPk}
        description: "Canonical attribute mapped from source '${m.sourceField}' with ${(m.score * 100).toFixed(0)}% confidence score."
        classification: ${isPii ? 'PII_RESTRICTED (Masked in non-prod)' : 'INTERNAL_CONFIDENTIAL'}
        tags:
          - "semantra:source_column:${m.sourceField}"
          - "semantra:mapping_type:${m.mappingType || 'exact'}"`;
}).join('\n')}

# ------------------------------------------------------------------------------
# Data Quality & Integrity Invariants (Automated CI/CD Validation)
# ------------------------------------------------------------------------------
quality:
  type: great_expectations
  rules:
${assertionResults.map(a => `    - name: "${a.name}"
      expression: "${a.rule}"
      severity: "${a.status === 'warning' ? 'warning' : 'error'}"`).join('\n')}`;
  };

  const getGitHubActionsWorkflowYaml = () => {
    return `# ==============================================================================
# GitHub Actions CI/CD Pipeline: Data Contract & Semantic Verification
# Generated by: Semantra Semantic Control Plane
# Path: .github/workflows/data_contract_ci.yml
# ==============================================================================
name: Semantra Data Contract & Transformation CI
...
jobs:
  verify-data-contract:
    name: Verify OpenDataContract Specification & Invariants
    ...
    steps:
      ...
      - name: Install Data Contract CLI & dbt
        run: |
          pip install datacontract-cli dbt-snowflake dbt-databricks pytest pydantic

      - name: 1. Validate OpenDataContract v2.2.0 Syntax & Linter
        run: |
          datacontract lint contracts/datacontract.yaml

      - name: 2. Test Contract against Target Schema (Breaking Change Gate)
        run: |
          datacontract test --schema contracts/datacontract.yaml

      - name: 3. Compile dbt Models (Databricks / Snowflake)
        run: |
          dbt compile --project-dir dbt_project/ --profiles-dir .

      - name: 4. Execute Semantra Invariant Assertion Suite
        run: |
          pytest tests/test_transformation_invariants.py -v

      - name: 5. Post Steward Compliance Badge to Pull Request
        if: always()
        run: |
          echo "### 🛡️ Semantra Data Governance Gate: PASSED" >> $GITHUB_STEP_SUMMARY
          echo "- **Data Contract:** contracts/datacontract.yaml" >> $GITHUB_STEP_SUMMARY
          echo "- **SLA Freshness:** ${contractFreshness}" >> $GITHUB_STEP_SUMMARY
          echo "- **Breaking Changes:** Blocked (Mode: ${contractSchemaEvolution})" >> $GITHUB_STEP_SUMMARY`;
  };

  const handleSimulateGitPr = () => {
    setGitPrStatus('submitting');
    setTimeout(() => {
      setGitPrStatus('created');
      setCreatedPrNumber(Math.floor(100 + Math.random() * 900));
    }, 1400);
  };

  const handleValidateContract = () => {
    setIsContractValidating(true);
    setTimeout(() => {
      setIsContractValidating(false);
      setContractValidationSuccess(true);
    }, 900);
  };
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          {onNavigateToHITL && (
            <button
              ...
              className="px-4 py-2.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-all font-sans bg-indigo-50 hover:bg-indigo-100 text-indigo-800 border border-indigo-200 shadow-xs cursor-pointer"
              title="Test batch ingestion and dry-run exceptions in the Human-in-the-Loop Execution Console"
            >
              <ShieldCheck className="w-4 h-4 text-indigo-600" />
              <span>HITL Batch Execution</span>
              <span className="px-1.5 py-0.2 text-[9px] font-mono bg-indigo-200/70 text-indigo-900 rounded border border-indigo-300 font-bold">
                Step 6
              </span>
            </button>
          )}
          ...
          <button
            onClick={() => setIsGitOpsModalOpen(true)}
            className="px-4 py-2.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-all font-sans bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-700 shadow-sm cursor-pointer"
            title="Automate CI/CD Pull Request to GitHub/GitLab with Data Contracts & dbt models"
          >
            <GitPullRequest className="w-4 h-4 text-emerald-400" />
            <span>GitOps / CI/CD (PR)</span>
            <span className="px-1.5 py-0.2 text-[9px] font-mono bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/40">
              Auto-PR
            </span>
          </button>
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-all font-sans ${
              isPublished
                ? 'bg-emerald-500 text-white cursor-default'
                : 'bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold shadow-sm cursor-pointer'
            }`}
          >
            {isPublished ? (
              <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
            {/* Header Tabs */}
            <div className="p-3.5 border-b border-slate-200 bg-slate-50/80 flex flex-col gap-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="hidden md:inline-flex px-2 py-0.5 rounded text-[10px] font-mono font-medium bg-slate-200/70 text-slate-600 border border-slate-300/60">
                    10 Enterprise Targets
                  </span>
                </div>
                
                {/* Active target badge display */}
                <div className="text-[11px] font-mono text-slate-500 flex items-center gap-1.5 self-start sm:self-auto">
                  ...
                  <span className="font-bold text-slate-800 bg-white px-2 py-0.5 rounded border border-slate-200 shadow-xs">
                    {activeCodeTab === 'datacontract' && 'OpenDataContract (ODC v2.2.0)'}
                    {activeCodeTab === 'pandas' && 'Pandas DataFrame (Python 3.11+)'}
                    {activeCodeTab === 'pyspark' && 'Databricks PySpark DataFrame'}
                    {activeCodeTab === 'dbt' && 'dbt Semantic Model (SQL)'}
                    {activeCodeTab === 'dbt_test' && 'dbt Schema Tests (YAML)'}
                    {activeCodeTab === 'great_expectations' && 'Great Expectations Python Suite'}
                    {activeCodeTab === 'sql_audit' && 'SQL Data Quality Assertions'}
                    {activeCodeTab === 'talend' && 'Talend Open Studio (XML)'}
                    {activeCodeTab === 'azure_sql' && 'T-SQL Stored Procedure (Azure SQL)'}
                    {activeCodeTab === 'openlineage' && 'OpenLineage Provenance Event (DataHub/Marquez)'}
                  </span>
                </div>
              </div>

              {/* Segmented Grouped Toolbar */}
              <div className="flex flex-wrap items-center gap-2.5 pt-0.5">
                {/* GROUP 1: Governance & Lineage */}
                <div className="flex items-center bg-indigo-50/60 p-1 rounded-lg border border-indigo-200/70 gap-1 shadow-xs">
                  <span className="px-2 py-0.5 text-[10px] font-mono uppercase font-bold tracking-wider text-indigo-700 flex items-center gap-1 select-none">
                    <FileCheck2 className="w-3 h-3 text-indigo-600" />
                    <span className="hidden sm:inline">Contracts &amp; Lineage</span>
                    <span className="sm:hidden">Gov</span>
                  </span>
                  <div className="h-4 w-px bg-indigo-200/80 mx-0.5" />
                  <button
                    onClick={() => setActiveCodeTab('datacontract')}
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 ${
                      activeCodeTab === 'datacontract'
                        ? 'bg-indigo-600 text-white font-bold shadow-xs'
                        : 'text-indigo-800 hover:text-indigo-950 hover:bg-indigo-100/60 font-medium'
                    }`}
                    title="OpenDataContract Specification (ODC v2.2.0) with SLA policies"
                  >
                    Data Contract (ODC)
                  </button>
                  <button
                ...
                {/* GROUP 2: Pipelines & ETL */}
                <div className="flex items-center bg-slate-100/90 p-1 rounded-lg border border-slate-200/80 gap-1 shadow-xs">
                  <span className="px-2 py-0.5 text-[10px] font-mono uppercase font-bold tracking-wider text-slate-600 flex items-center gap-1 select-none">
                    <Terminal className="w-3 h-3 text-slate-500" />
                    <span className="hidden sm:inline">Pipelines &amp; ETL</span>
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${
                      activeCodeTab === 'pandas' ? 'bg-white text-slate-900 shadow-xs font-bold border border-slate-300/80' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                    }`}
                    title="Pandas DataFrame (Python 3.11+)"
                  >
                    Pandas
                  </button>
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${
                      activeCodeTab === 'pyspark' ? 'bg-white text-slate-900 shadow-xs font-bold border border-slate-300/80' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                    }`}
                    title="Databricks PySpark DataFrame"
                  >
                    PySpark
                  </button>
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${
                      activeCodeTab === 'dbt' ? 'bg-white text-slate-900 shadow-xs font-bold border border-slate-300/80' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                    }`}
                    title="dbt SQL transformation model"
                  >
                    dbt SQL
                  </button>
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${
                      activeCodeTab === 'azure_sql' ? 'bg-white text-slate-900 shadow-xs font-bold border border-slate-300/80' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                    }`}
                    title="Azure SQL Server T-SQL Stored Procedure"
                  >
                    Azure SQL
                  </button>
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${
                      activeCodeTab === 'talend' ? 'bg-white text-slate-900 shadow-xs font-bold border border-slate-300/80' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                    }`}
                    title="Talend Open Studio XML Job Item"
                  >
                    Talend XML
                  </button>
                </div>

                {/* GROUP 3: Quality & Tests */}
                <div className="flex items-center bg-emerald-50/60 p-1 rounded-lg border border-emerald-200/70 gap-1 shadow-xs">
                  <span className="px-2 py-0.5 text-[10px] font-mono uppercase font-bold tracking-wider text-emerald-800 flex items-center gap-1 select-none">
                    <ShieldCheck className="w-3 h-3 text-emerald-600" />
                    <span className="hidden sm:inline">Quality &amp; Tests</span>
                    <span className="sm:hidden">DQ</span>
                  </span>
                  <div className="h-4 w-px bg-emerald-200/80 mx-0.5" />
                  <button
                    onClick={() => setActiveCodeTab('dbt_test')}
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${
                      activeCodeTab === 'dbt_test'
                        ? 'bg-emerald-600 text-white font-bold shadow-xs'
                        : 'text-emerald-800 hover:text-emerald-950 hover:bg-emerald-100/60 font-medium'
                    }`}
                    title="Auto-generated dbt schema tests (YAML)"
                  >
                    dbt Tests
                  </button>
                  <button
                    ...
                    title="Great Expectations Python validation suite"
                  >
                    Great Expectations
                  </button>
                  <button
                    ...
                    title="Automated SQL Data Quality Audits"
                  >
                    SQL DQ Audit
                  </button>
                </div>
            ...
            {/* OpenDataContract SLA & Governance Policy Configurator Bar */}
            {activeCodeTab === 'datacontract' && (
              <div className="p-4 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border-b border-indigo-900/60 text-xs space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <FileCheck2 className="w-4 h-4 text-emerald-400" />
                      <span className="font-bold text-white font-sans text-sm">OpenDataContract (ODC v2.2.0) Architecture Policy</span>
                    ...
                    <p className="text-[11px] text-slate-300 font-sans">
                      Defines official consumer-producer contracts, data freshness SLAs, and quarantine routing before shipping to Databricks/Snowflake.
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    >
                      {isContractValidating ? (
                        <>
                          ...
                          <span>Validating ODC Spec...</span>
                        </>
                      ) : (
                        <>
                          <CheckCircle className="w-3.5 h-3.5 text-emerald-300" />
                          <span>Validate ODC Spec</span>
                        </>
                      )}
                    </button>
                    ...
                    <button
                      onClick={() => {
                        const blob = new Blob([getDataContractYaml()], { type: 'text/yaml' });
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `datacontract_${selectedPreset ? selectedPreset.toLowerCase().replace(/[^a-z0-9_]/g, '_') : 'canonical'}.yaml`;
                        a.click();
                ...
                {/* Interactive SLA & Policy Controls */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 pt-2 border-t border-indigo-900/50">
                  <div className="bg-slate-900/80 border border-indigo-900/40 rounded-lg p-2 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold flex items-center gap-1">
                      <Clock className="w-3 h-3 text-indigo-400" /> SLA Freshness
                    </label>
                    <select
                      value={contractFreshness}
                      onChange={(e) => setContractFreshness(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-700 text-[11px] text-slate-200 rounded p-1 font-mono focus:outline-none focus:border-indigo-500"
                    >
                      <option value="15m max latency (Near Real-Time)">15m Max Latency (Streaming/Microbatch)</option>
                      <option value="1h max latency (Hourly Ingestion)">1h Max Latency (Hourly Batch)</option>
                      <option value="Daily by 06:00 UTC (Executive Reporting)">Daily by 06:00 UTC (Reporting)</option>
                    </select>
                  </div>

                  <div className="bg-slate-900/80 border border-indigo-900/40 rounded-lg p-2 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold flex items-center gap-1">
                      <Sliders className="w-3 h-3 text-indigo-400" /> Evolution Mode
                    </label>
                    <select
                      value={contractSchemaEvolution}
                      onChange={(e) => setContractSchemaEvolution(e.target.value as any)}
                      className="w-full bg-slate-950 border border-slate-700 text-[11px] text-slate-200 rounded p-1 font-mono focus:outline-none focus:border-indigo-500"
                    >
                      <option value="strict">strict (Breaking Changes Forbidden)</option>
                      <option value="additive_only">additive_only (New Nullable Fields Allowed)</option>
                      <option value="compatible">compatible (Backward Compatible Only)</option>
                    </select>
                  </div>

                  <div className="bg-slate-900/80 border border-indigo-900/40 rounded-lg p-2 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold flex items-center gap-1">
                      <ShieldCheck className="w-3 h-3 text-emerald-400" /> Availability SLA
                    </label>
                    <select
                      value={contractAvailability}
                      onChange={(e) => setContractAvailability(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-700 text-[11px] text-slate-200 rounded p-1 font-mono focus:outline-none focus:border-indigo-500"
                    >
                      <option value="99.95% uptime">99.95% Uptime (Mission Critical)</option>
                      <option value="99.9% uptime">99.9% Uptime (Standard Tier)</option>
                      <option value="99.5% uptime">99.5% Uptime (Internal Analytics)</option>
                    </select>
                  </div>

                  <div className="bg-slate-900/80 border border-indigo-900/40 rounded-lg p-2 space-y-1">
                    <label className="text-[10px] font-mono text-slate-400 uppercase font-semibold flex items-center gap-1">
                      <BookOpen className="w-3 h-3 text-indigo-400" /> Domain Owner
                    </label>
                    <input
                      type="text"
                      value={contractDomain}
                      onChange={(e) => setContractDomain(e.target.value)}
                      placeholder="e.g. Finance & Billing Core"
                      className="w-full bg-slate-950 border border-slate-700 text-[11px] text-slate-200 rounded p-1 font-sans focus:outline-none focus:border-indigo-500"
                    />
                  </div>
                </div>

                {contractValidationSuccess && (
                  <div className="flex items-center justify-between bg-emerald-950/60 border border-emerald-800/60 rounded-lg px-3 py-1.5 text-emerald-300 text-[11px] font-sans">
                    <div className="flex items-center gap-1.5">
                      <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
                      <span><strong>ODC v2.2.0 Spec Valid:</strong> Verified schema invariants, SLA metrics, and field data types.</span>
                    </div>
                    <span className="text-[10px] font-mono bg-emerald-900/80 px-1.5 py-0.5 rounded text-emerald-200 border border-emerald-700">
                      datacontract-cli Ready
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden text-slate-100">
            ...
            <div className="p-5 border-b border-slate-800 bg-slate-950 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-indigo-600 flex items-center justify-center shadow-md">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                      Zero-Execution Compiler
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 font-sans">
                    Push Semantra Data Contract &amp; compiled dbt models directly to your Git repository as an automated Pull Request.
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-4 bg-slate-900/90 border-b border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="space-y-1">
                <label className="font-mono text-[10px] text-slate-400 uppercase font-semibold flex items-center gap-1">
                  <FolderGit2 className="w-3.5 h-3.5 text-indigo-400" /> Git Provider
                </label>
                <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800">
                  {(['github', 'gitlab', 'azure_devops'] as const).map(p => (
                    <button
                      key={p}
                      type="button"
                      onClick={() => setGitProvider(p)}
                      className={`flex-1 py-1 rounded text-[11px] font-medium capitalize transition-all cursor-pointer ${
                        gitProvider === p ? 'bg-indigo-600 text-white font-bold shadow' : 'text-slate-400 hover:text-slate-200'
                      }`}
                    >
                      {p === 'azure_devops' ? 'Azure DevOps' : p}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="font-mono text-[10px] text-slate-400 uppercase font-semibold flex items-center gap-1">
                  <GitBranch className="w-3.5 h-3.5 text-emerald-400" /> Target Repository
                </label>
                <input
                  type="text"
                  value={gitRepo}
              ...
              <div className="space-y-1">
                <label className="font-mono text-[10px] text-slate-400 uppercase font-semibold flex items-center gap-1">
                  <GitBranch className="w-3.5 h-3.5 text-indigo-400" /> Working Branch
                </label>
                <input
                  type="text"
                  value={gitBranch}
            ...
            {/* Sub-tabs: CI/CD Workflow vs PR Preview vs Packaged Files */}
            <div className="px-5 pt-3 bg-slate-950 flex border-b border-slate-800 gap-4 text-xs font-sans">
              <button
                onClick={() => setGitSelectedTab('workflow')}
                className={`pb-2.5 font-semibold transition-all border-b-2 flex items-center gap-1.5 cursor-pointer ${
                  gitSelectedTab === 'workflow'
                    ? 'border-indigo-500 text-white font-bold'
                    : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
              >
                <FileCode2 className="w-3.5 h-3.5" />
                <span>GitHub Actions CI/CD (.github/workflows/data_contract_ci.yml)</span>
              </button>
              <button
                onClick={() => setGitSelectedTab('pr_preview')}
                className={`pb-2.5 font-semibold transition-all border-b-2 flex items-center gap-1.5 cursor-pointer ${
                  gitSelectedTab === 'pr_preview'
                    ? 'border-indigo-500 text-white font-bold'
                    : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
              >
                <GitPullRequest className="w-3.5 h-3.5" />
                <span>Pull Request Description &amp; Bundle</span>
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 bg-slate-950 space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-300 font-sans">
                      Automated CI Pipeline checks contract syntax with <code>datacontract-cli</code> and validates dbt models before merging.
                    </span>
                    <button
                      onClick={() => navigator.clipboard.writeText(getGitHubActionsWorkflowYaml())}
                    ...
                    >
                      ...
                      <span>Copy CI YAML</span>
                    </button>
                  </div>
                  <pre className="p-4 bg-slate-900 border border-slate-800 rounded-xl text-xs font-mono text-emerald-400 overflow-x-auto whitespace-pre max-h-80">
                    {getGitHubActionsWorkflowYaml()}
                  </pre>
                </div>
              ) : (
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                        Target: {gitRepo} ({gitBranch})
                      </span>
                    </div>
                    <p className="text-slate-300 text-xs leading-relaxed">
                      Generated by Semantra Semantic Control Plane. Contains validated canonical field mappings, OpenDataContract v2.2.0 spec, and native dbt transformation models.
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h4 className="font-bold text-slate-200 uppercase tracking-wider text-[11px] font-mono">
                      Packaged Pull Request Artifacts (Zero-Execution Code)
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-3 space-y-1">
                        <div className="flex items-center justify-between font-mono text-xs">
                          <span className="text-indigo-300 font-bold">contracts/datacontract.yaml</span>
                          <span className="text-[10px] text-slate-400">ODC v2.2.0</span>
                        </div>
                        <p className="text-[11px] text-slate-400">
                          Data SLA freshness ({contractFreshness}), {contractSchemaEvolution} evolution policy, and domain stewardship ownership.
                        </p>
                      </div>

                      <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-3 space-y-1">
                        <div className="flex items-center justify-between font-mono text-xs">
                          <span className="text-emerald-400 font-bold">models/stg_canonical.sql</span>
                          <span className="text-[10px] text-slate-400">dbt SQL</span>
                        </div>
                        <p className="text-[11px] text-slate-400">
                          Native SQL compilation with {mappings.length} mapped attributes and explicit type casting.
                        </p>
                      </div>

                      <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-3 space-y-1">
                        <div className="flex items-center justify-between font-mono text-xs">
                          <span className="text-emerald-400 font-bold">models/schema.yml</span>
                          <span className="text-[10px] text-slate-400">dbt Tests</span>
                        </div>
                        <p className="text-[11px] text-slate-400">
                          Automated schema tests (not_null, unique, accepted_values) for Snowflake/Databricks.
                        </p>
                      </div>

                      <div className="bg-slate-900/80 border border-slate-800 rounded-lg p-3 space-y-1">
                        <div className="flex items-center justify-between font-mono text-xs">
                          <span className="text-indigo-300 font-bold">.github/workflows/data_contract_ci.yml</span>
                          <span className="text-[10px] text-slate-400">GitHub Actions</span>
                        </div>
                        <p className="text-[11px] text-slate-400">
                          CI linter &amp; automated test runner gate for breaking changes.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {/* PR Status Simulation Card */}
              {gitPrStatus === 'created' && (
                <div className="p-4 bg-gradient-to-r from-emerald-950/80 via-slate-900 to-indigo-950/80 border border-emerald-500/50 rounded-xl space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <CheckCircle className="w-5 h-5 text-emerald-400" />
                      <span className="font-bold text-white text-sm">
                        Pull Request #{createdPrNumber || 142} Created on {gitProvider === 'azure_devops' ? 'Azure DevOps' : gitProvider === 'gitlab' ? 'GitLab' : 'GitHub'}!
                      </span>
                    </div>
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                      PR #{(createdPrNumber || 142)} OPEN
                    </span>
                  </div>

                  <div className="text-xs text-slate-300 space-y-1 font-mono">
                    <div>Branch: <span className="text-indigo-300">{gitBranch}</span> &rarr; <span className="text-white">main</span></div>
                    <div>Repo: <span className="text-slate-200">https://{gitProvider}.com/{gitRepo}/pull/{(createdPrNumber || 142)}</span></div>
                  </div>

                  <div className="p-2.5 bg-slate-950/80 rounded-lg border border-slate-800 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      <CheckCheck className="w-4 h-4 text-emerald-400" />
                      <span className="text-slate-300 font-sans">
                        CI Status: <strong>3/3 Checks Passing</strong> (datacontract-lint ✓, dbt-compile ✓, steward-signoff ✓)
                      </span>
                    </div>
            ...
            {/* Modal Footer */}
            <div className="p-4 border-t border-slate-800 bg-slate-950 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div className="text-slate-400 font-sans">
                💡 <em>Zero Execution Footprint: Compiled code is sent to your Git repo for execution on your own cloud runners.</em>
              </div>
              ...
              <div className="flex items-center gap-2 justify-end">
                ...
                {gitPrStatus !== 'created' ? (
                  <button
                    type="button"
                    onClick={handleSimulateGitPr}
                    disabled={gitPrStatus === 'submitting'}
                    className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-indigo-600 hover:from-emerald-400 hover:to-indigo-500 text-slate-950 font-bold rounded-lg transition-all flex items-center gap-1.5 shadow cursor-pointer disabled:opacity-50"
                  >
                    {gitPrStatus === 'submitting' ? (
                      <>
                        <RefreshCw className="w-4 h-4 animate-spin text-slate-950" />
                        <span>Creating PR on {gitProvider}...</span>
                      </>
                    ) : (
                      <>
                        <GitPullRequest className="w-4 h-4 text-slate-950" />
                        <span>⚡ Create Pull Request ({gitProvider === 'azure_devops' ? 'Azure' : gitProvider})</span>
                      </>
                    )}
                  </button>
                ) : (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const a = document.createElement('a');
                      a.href = url;
                      a.download = `semantra_gitops_bundle_${selectedPreset || 'canonical'}.yaml`;
                      a.click();
                      URL.revokeObjectURL(url);
                    }}
                    className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer"
                  >
                    <Download className="w-4 h-4" />
                    <span>Download PR Bundle (.yaml)</span>
                  </button>
                )}
              </div>
            </div>

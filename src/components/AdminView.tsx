  ...
  return (
    <div className="space-y-6 text-slate-100 font-sans">
      ...
      {activeSection === 'Overlays & Runtime' && (
        <div className="space-y-6 pt-2 text-slate-200 font-sans">
          
          {/* Top Informational Banner on Multi-Overlay Stacking */}
          <div className="bg-slate-900 border border-emerald-500/30 rounded-xl p-4 text-xs font-mono text-slate-200 flex items-start gap-3 shadow-sm">
            ...
            <div className="space-y-1">
              ...
              <p className="text-slate-300 text-[11px] leading-relaxed">
                <strong className="text-emerald-400 font-bold">YES!</strong> In the Semantra architecture, multiple overlay files (e.g., SAP, HR, QuickBooks) can be <strong className="text-emerald-300 font-bold">active simultaneously</strong> in the runtime stack. They are applied in priority order: adding multiple files combines rules seamlessly without altering base canonical definitions.
              </p>
            </div>
          </div>
                ...
                return (
                  <div 
                    ...
                    className={`border rounded-xl p-4 transition-all space-y-3 font-mono text-xs ${
                      isActive 
                        ? 'bg-slate-950 text-white border-emerald-500/40 shadow-sm ring-1 ring-emerald-500/20' 
                  ...
                  >
                    ...
                    <div>
                      <h4 className={`font-bold text-sm truncate ${isActive ? 'text-white' : 'text-slate-200'}`} title={ov.title}>{ov.title}</h4>
      ...
      {activeSection === 'Stewardship' && (
        <div className="space-y-6 pt-2 text-slate-800">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm space-y-4 font-sans text-slate-200">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
            ...
            <div className="space-y-3 pt-2 font-mono text-xs">
              {stewardshipItems.filter(i => i.status === 'ready_for_approval').length === 0 ? (
                ...
                stewardshipItems
                  ...
                  .map((item) => (
                    <div key={item.id} className="border border-slate-800 rounded-lg p-4 space-y-3 bg-slate-950 relative">
                      <div className="flex justify-between items-start gap-2">
                        <div>
                          <span className="text-[10px] text-slate-400 font-bold uppercase block">PROPOSED ALIAS</span>
                          <span className="text-sm font-bold text-white">{item.proposedAlias}</span>
                        </div>
                      ...
                      <div className="space-y-1 text-slate-300 text-xs">
                        <p><strong className="text-slate-400">Target Concept:</strong> <span className="text-indigo-400 font-bold">{item.conceptName}</span></p>
                        <p><strong className="text-slate-400">Source Context:</strong> <span className="text-slate-200">{item.sourceContext}</span></p>
                        {item.reviewNote && (
                          <div className="mt-1 bg-slate-900 border border-slate-800 p-2 rounded text-[11px] italic text-slate-300 font-sans">
                            "{item.reviewNote}"
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2 justify-end pt-2 border-t border-slate-800">
                        <button
                          ...
                          className="px-3 py-1.5 border border-slate-700 bg-slate-900 text-slate-300 hover:text-rose-300 hover:bg-rose-950/40 text-xs font-bold rounded flex items-center gap-1 transition-colors"
                        >
                          <X className="w-3.5 h-3.5" /> Reject
                        </button>
                        <button
                          onClick={() => handleResolveStewardship(item.id, 'approved')}
                          className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded flex items-center gap-1 shadow-sm transition-colors"
                        >
